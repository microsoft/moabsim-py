...
#Control type 1a definition: immediate reward and single state dependent constraints: in this problem |x| < Squareside & |y| < Squareside
...
const SquareSide = (0.6*RadiusOfPlate)/Math.Sqrt(2) # square inside plate
    ...
    concept MoveToCenter(input): SimAction {
        curriculum {
            ...
            goal (State: ObservableState) {
                avoid `Out of square x`:
                    Math.Abs(State.ball_x)
                    in Goal.RangeAbove(SquareSide)
                avoid `Out of square y`:
                    Math.Abs(State.ball_y)
                    in Goal.RangeAbove(SquareSide)
